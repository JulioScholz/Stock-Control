...
public class Stock {
    ...
    public Product update_qty(String product,  int new_qty) throws SQLException {
        ...
        try {
            myStmt = connection.prepareStatement ("SELECT quantidade,preco,nome FROM produtos WHERE id = ?%");
        } catch (SQLException e) {
            e.printStackTrace ();
            return null;
        }
        myStmt.setInt (1, Integer.parseInt (product));

        ResultSet rs = myStmt.executeQuery ();
        rs.next ();
        Product aux = new Product (product, rs.getString (2), rs.getInt (0), rs.getFloat (0));
        rs.close ();
        myStmt.close ();
        connection.close ();
        aux.setQuantidadeAux (aux.getQuantidade () - new_qty);
